...
`type` キーワードでレコード型を定義する。フィールドはカンマ `,` で区切る。
...
型名を大文字で始まるコンストラクタとして使い、レコード値を生成する。
...
バリアント型と同様、レコード型のコンストラクタは定義されたモジュールのスコープに属する。
...
type_def    ::= 'type' IDENT TYVAR* '=' '{' variant_arm+ '}'
              | 'type' IDENT TYVAR* '=' '{' field_def (',' field_def)* ','? '}'
